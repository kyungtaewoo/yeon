 ...
 *  3) 로그인 → acceptInvite. 성공 시 /friend-detail?id=INVITE_ID 상세로.
...
export function InviteWelcome() {
  ...
  // Capacitor 진입은 /invite/__placeholder__?code= 로 들어오고, Vercel 직접 진입은
  // /invite/[code] path segment 로 들어옴. search 우선, params fallback. placeholder
  // 값은 실제 코드가 아니므로 무시.
  const codeFromSearch = search.get("code") ?? "";
  const codeFromParam = String(params.code ?? "");
  const code =
    codeFromSearch || (codeFromParam !== "__placeholder__" ? codeFromParam : "");
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      // 친구 상세 — dynamic route 대신 query string 단일 경로 (/friend-detail?id=)
      router.replace(`/friend-detail?id=${encodeURIComponent(invite.id)}`);
    } catch (e) {
